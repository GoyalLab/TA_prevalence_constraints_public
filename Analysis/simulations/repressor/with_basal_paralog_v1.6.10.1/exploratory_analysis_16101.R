...
write.csv(allstats101 %>% mutate(version = '1.6.10.1'), file = paste0(plotdir101, 'summary_stats.csv'), row.names = F)
...
classes_sankey101_B1forfig <- ggplot(basic_class_assignment_all_forSankey101 %>%
                                      filter(product == 'B1',
  ...
  xlab('Mutated alleles') +
  ylab('Number of parameter sets')
ggsave(classes_sankey101_B1forfig, file = paste0(plotdir101, 'stats_class_assignment_check_v', as.character(anver),'/classes_sankey_B1formainfig.pdf'))
...
temp_class_for_tree101_rename <- temp_class_for_tree101 %>%
...
temp.tree <- ctree(is_robust ~ basal_nitc_on_ratio + onbasalA1_off_ratio + A1_Aprime1_addoff_ratio + 
                     A1_Aprime_Bonrate_ratio + r_prod_on + r_addon_byA1_B1 + r_onbasal_A1 + r_onbasal_Aprime_ratio, 
                   data = temp_class_for_tree101_rename,
                   control = ctree_control(alpha = 0.01))
...
for (aci in 1:length(genos)) {
  ...
  for (proi in 1:length(prods)) {
    ...
    tempforcorr1 <- allstats_full101 %>% 
      ...
      pivot_longer(names_to = 'statistic', values_to = 'stat_value', cols = log10_mean_product1:entropy) %>%
      pivot_longer(names_to = 'parameter', values_to = 'param_value', cols = basal_nitc_on_ratio:r_onbasal_Aprime_ratio)
    ggsave(sc1, file = paste0(plotdir101, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
    ...
    if(nrow(tempforcorr1)>0){
      sc1d <- ggplot(tempforcorr, aes(param_value, stat_value)) +
        ...
        ylab('statistic value') +
        ggtitle(paste0('Statistic vs parameter value\nGene: ', pro, ', Genotype: ', as.character(ac), ' mutated alleles'))
      ggsave(sc1d, file = paste0(plotdir101, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
      ...
      ggsave(sc2, file = paste0(plotdir101, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
      ggsave(sc2d, file = paste0(plotdir101, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
    }
  }
}
...
tempforcorr1 <- allstats_full101 %>% 
  ...
  filter(statistic == 'bimodality_coef',
         parameter %in% c('A1_Aprime1_addon_ratio', 'A1_Aprime_prodon_ratio', 'basal_nitc_on_ratio', 'r_onbasal_Aprime_ratio'))
ggsave(sc1, file = paste0(plotdir101, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 16, height = 4)
...
if(nrow(tempforcorr1)>0){
  sc1d <- ggplot(tempforcorr, aes(param_value, stat_value)) +
    ...
    ylab('statistic value') +
    ggtitle(paste0('Statistic vs parameter value\nGene: ', pro, ', Genotype: ', as.character(ac), ' mutated alleles'))
  ggsave(sc1d, file = paste0(plotdir101, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 16, height = 4)
  ...
  ggsave(sc2, file = paste0(plotdir101, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 16, height = 4)
  ggsave(sc2d, file = paste0(plotdir101, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 16, height = 4)
}
...
write.table(cortab, file = paste0(plotdir101, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_pearsonCor.txt'), quote = F, row.names = F)

...
write.csv(allstats111 %>% mutate(version = '1.6.11.1'), file = paste0(plotdir111, 'summary_stats.csv'), row.names = F)
...
allstats_full111 %<>% mutate(skewness = ifelse(is.na(skewness), 0, skewness))

compared_stats111 <- allstats_full111 %>% 
...
unistats111<-unique(compared_stats111$stat)

for (st in unistats111) {
  
  pvs1 <- ggplot(allstats_full111 %>% inner_join(lhs_sets_full111, by = c('version', 'paramset')) %>% filter(product == 'B1', Hill_coefficient_n<5)) + 
  ...
  pvs2 <- ggplot(allstats_full111 %>% inner_join(lhs_sets_full111, by = c('version', 'paramset')) %>% filter(product == 'B1', Hill_coefficient_n<5)) + 
  ...
  ggsave(pvs1, file = paste0(plotdir111, 'PerGenotype_', st, '_vs_mean.pdf'), width = 16, height = 8) 
  ggsave(pvs2, file = paste0(plotdir111, 'PerGenotype_', st, '_vs_logmean.pdf'), width = 16, height = 8) 
  ggsave(pvs3, file = paste0(plotdir111, 'WTMUT_', st, '_vs_logmean.pdf'), width = 4, height = 4) 
}


# LOESS
loess_fitted_allstats_all111 <- allstats_full111
for (stat in unistats111[unistats111 != 'mean_product']) {
  ...
  loess_fitted_allstats_all111 %<>% left_join(as_tibble(statdat) %>% dplyr::select(-mean_product), by = c('version', 'paramset', 'mutated_alleles', 'product'))
  # 
  # cat('sliding window normalizing...\n') # do this per-gene over all genotypes, rather than all genes over all genotypes...
  # statdat1 <- sliding_window_normalize(as_tibble(statdat) %>% filter(mean_product>10), 'mean_product', paste0(stat,'_residual'), 50)
  # 
  
  loess_fitted_allstats_all111 %<>% left_join(statdat1 %>% dplyr::select(-c('mean_product', paste0(stat,'_residual'), paste0(stat,'_fitted'))), by = c('version', 'paramset', 'mutated_alleles', 'product'))
...
write.csv(loess_fitted_allstats_all111, file = paste0(plotdir111, 'loess_fitted_allstats_all_snwRadius100.csv'), quote = F, row.names = F)

# load as needed
# loess_fitted_allstats_all111 <- as_tibble(read.csv(paste0(plotdir111, 'loess_fitted_allstats_all_snwRadius100.csv'), header=T, stringsAsFactors = F))
...
for (stat in unistats111[unistats111 != 'mean_product']) {
  ...
  for (gene in c('A1', 'Anonsense1', 'Aprime1', 'B1')) {
    ...
    statdatA = loess_fitted_allstats_all111 %>%
...
basic_class_assignment_all111 <- loess_fitted_allstats_all111 %>%
...
basic_class_assignment_all_forSankey111 <- basic_class_assignment_all111 %>%
...
set.seed(73245)
basic_class_assignment_all_forSankey_forsamples111 <- basic_class_assignment_all111 %>%
  dplyr::select(version, paramset, product, mutated_alleles, class_assignment)  

classes111 = unique(basic_class_assignment_all111$class_assignment)

if(!dir.exists(paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver)))) {
  dir.create(paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver)))
}

classes_sankey <- ggplot(basic_class_assignment_all_forSankey111, aes(x = mutated_alleles, y=Freq,
  ...
  theme_classic() +
  theme(legend.position = 'none') +
  ggtitle('Class assignments before and after mutations\nNegative regulation, log-sampled parameters') +
  xlab('Mutated alleles') +
  ylab('Number of parameter sets')
ggsave(classes_sankey, file = paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/classes_sankey.pdf'))

classes_sankey111_B1forfig <- ggplot(basic_class_assignment_all_forSankey111 %>%
                                       filter(product == 'B1',
  ...
  geom_stratum(alpha = 0.5) +
  geom_text(stat = 'stratum', size = 3) + 
  scale_fill_manual(values = RColorBrewer::brewer.pal(name='Set2', n = 5)[c(1,3,4,5)]) +
  theme_classic() +
  theme(legend.position = 'none') +
  ggtitle('Class assignments before and after mutation\nGene B1, Negative regulation, log-sampled parameters\nWith basal paralog expression') +
  xlab('Mutated alleles') +
  ylab('Number of parameter sets')
ggsave(classes_sankey111_B1forfig, file = paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/classes_sankey_B1formainfig.pdf'))


basic_class_assignment_all_forpie111 <- basic_class_assignment_all111 %>%
  group_by(mutated_alleles, product, class_assignment) %>%
  summarise(nSets = length(product))

classes_pies <- ggplot(basic_class_assignment_all_forpie111, aes(x="", y=nSets, fill=class_assignment)) +
  geom_bar(stat='identity', width=1, color='white') +
  coord_polar('y', start=0) +
  facet_grid(product~mutated_alleles) +
  theme_void() +
  ggtitle('classes of all distributions in v1.6.11.1\neach gene in each genotype')
ggsave(classes_pies, file = paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/classes_pies.pdf'))
...
basic_class_assignment_all111_wide <- basic_class_assignment_all111 %>% 
...
classes_for_trees111 <- inner_join(basic_class_assignment_all111_wide, lhs_sets111, by = 'paramset')

temp_class_for_tree111 <- classes_for_trees111 %>%
  dplyr::select(colnames(lhs_sets111), `1_B1`) %>% ungroup() %>%
  dplyr::select(-paramset) %>%
  mutate(is_bimodal = ifelse(`1_B1` == 'bimodal', T, F)) %>%
  dplyr::select(-`1_B1`)

temp_class_for_tree111$is_bimodal <- as.factor(temp_class_for_tree111$is_bimodal)
...
temp_class_for_tree111 <- classes_for_trees111 %>%
  ...
  dplyr::select(colnames(lhs_sets111), is_robust) %>% ungroup() %>%
  dplyr::select(-paramset) 

temp_class_for_tree111$is_robust <- as.factor(temp_class_for_tree111$is_robust)

# rename for repressor ratios. addon = addoff; prodon = Bonrate
temp_class_for_tree111_rename <- temp_class_for_tree111 %>%
  dplyr::rename(A1_Aprime1_addoff_ratio = A1_Aprime1_addon_ratio,
                A1_Aprime_Bonrate_ratio = A1_Aprime_prodon_ratio)

temp.tree <- ctree(is_robust ~ basal_nitc_on_ratio + onbasalA1_off_ratio + A1_Aprime1_addoff_ratio + 
                     A1_Aprime_Bonrate_ratio + r_prod_on + r_addon_byA1_B1 + r_onbasal_A1, 
                   data = temp_class_for_tree111_rename,
                   control = ctree_control(alpha = 0.01))

pdf(paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/isRobust_unimodalsymmetric_tree.pdf'), width = 30, height = 10)
plot(temp.tree)
dev.off()
...
for (aci in 1:length(genos)) {
  ...
  for (proi in 1:length(prods)) {
    ...
    # sc1 <- ggplot(tempforcorr, aes(log10(param_value), stat_value)) + # lhs_sets file is already log-transformed
    sc1 <- ggplot(tempforcorr, aes(param_value, stat_value)) +
    ...
    tempforcorr1 <- allstats_full111 %>% 
      ...
      pivot_longer(names_to = 'statistic', values_to = 'stat_value', cols = log10_mean_product1:entropy) %>%
      pivot_longer(names_to = 'parameter', values_to = 'param_value', cols = basal_nitc_on_ratio:r_onbasal_A1)
    ggsave(sc1, file = paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
    
    if(nrow(tempforcorr1)>0){
      sc1d <- ggplot(tempforcorr, aes(param_value, stat_value)) +
        geom_point(alpha = 0.1, stroke = 0) +
        geom_density2d() +
        facet_grid(statistic ~ parameter, scales = 'free') + 
        theme_classic() +
        ...
        ylab('statistic value') +
        ggtitle(paste0('Statistic vs parameter value\nGene: ', pro, ', Genotype: ', as.character(ac), ' mutated alleles'))
      ggsave(sc1d, file = paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
      
      sc2 <- ggplot(tempforcorr1, aes(param_value, stat_value)) +
        geom_point(alpha = 0.1, stroke = 0) +
        facet_grid(statistic ~ parameter, scales = 'free') + 
        theme_classic() +
        xlab('log10(parameter value)') +
      ...
      sc2d <- ggplot(tempforcorr1, aes(param_value, stat_value)) +
        geom_point(alpha = 0.1, stroke = 0) +
        geom_density2d() +
        facet_grid(statistic ~ parameter, scales = 'free') + 
        theme_classic() +
      ...
      ggsave(sc2, file = paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
      ggsave(sc2d, file = paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
    }
  }
}
...
tempforcorr1 <- allstats_full111 %>% 
  ...
  filter(statistic == 'bimodality_coef',
         parameter %in% c('A1_Aprime1_addon_ratio', 'A1_Aprime_prodon_ratio', 'basal_nitc_on_ratio', 'r_onbasal_Aprime_ratio'))
ggsave(sc1, file = paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 16, height = 4)
...
if(nrow(tempforcorr1)>0){
  sc1d <- ggplot(tempforcorr, aes(param_value, stat_value)) +
    ...
    ylab('statistic value') +
    ggtitle(paste0('Statistic vs parameter value\nGene: ', pro, ', Genotype: ', as.character(ac), ' mutated alleles'))
  ggsave(sc1d, file = paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 16, height = 4)
  ...
  ggsave(sc2, file = paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 16, height = 4)
  ggsave(sc2d, file = paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 16, height = 4)
}
...
c_addon <- cor.test(tempforcortest1$bimodality_coef, tempforcortest1$A1_Aprime1_addon_ratio)
c_absaddon <- cor.test(tempforcortest1$bimodality_coef, abs(tempforcortest1$A1_Aprime1_addon_ratio))
c_prodon <- cor.test(tempforcortest1$bimodality_coef, tempforcortest1$A1_Aprime_prodon_ratio)
c_absprodon <- cor.test(tempforcortest1$bimodality_coef, abs(tempforcortest1$A1_Aprime_prodon_ratio))
c_nitc <- cor.test(tempforcortest1$bimodality_coef, tempforcortest1$basal_nitc_on_ratio)
c_absnitc <- cor.test(tempforcortest1$bimodality_coef, abs(tempforcortest1$basal_nitc_on_ratio))
c_primon <- cor.test(tempforcortest1$bimodality_coef, tempforcortest1$r_onbasal_Aprime_ratio)
c_absprimon <- cor.test(tempforcortest1$bimodality_coef, abs(tempforcortest1$r_onbasal_Aprime_ratio))
...
write.table(cortab, file = paste0(plotdir111, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_pearsonCor.txt'), quote = F, row.names = F)

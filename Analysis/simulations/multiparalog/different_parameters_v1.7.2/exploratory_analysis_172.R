...
# edit as needed
datadir172 <- '/Volumes/IAMYG2/grn_nitc_data/v1.7.2/samples/'
plotdir172 <- '/Volumes/IAMYG2/grn_nitc_data/v1.7.2/exploratory_analysis/'
setwd(datadir172)

if(!dir.exists(plotdir172)){
  dir.create(plotdir172)
}
paramsets172 <- 1:10000
lhs_sets172 <- as_tibble(read.csv('latinhyp_sampledSets.csv')) 
lhs_sets172 %<>%
  mutate(paramset = 1:nrow(lhs_sets172))

# calculate stats
allstats172 <- list()
allparams172 <- list()
for (paramset in paramsets172){
  ...
  species_sample <- species %>%
    ...
    dplyr::select(A1, Aprime1, Aprime2, Anonsense1, B1, paramset, time, mutated_alleles) %>%
  ...
  if(paramset %% 100 == 0) {
    ...
    #   theme_classic()
    # ggsave(dist_plot, file = paste0(plotdir172, 'distributions_q300_v1.7.2_paramset_', as.character(paramset), '.pdf'))
  }
  ...
  for (ma in c(0,1,2)) {
    ...
    for (gene in c('A1', 'Aprime1', 'Aprime2', 'Anonsense1', 'B1')) {
  ...
  if(is.null(dim(allstats172))) {
    allstats172 <- spstats
  } else {
    allstats172 %<>% bind_rows(spstats)
  }
  
  if(is.null(dim(allparams172))) {
    allparams172 <- params
  } else {
    allparams172 %<>% bind_rows(params)
  }
  
}
write.csv(allstats172 %>% mutate(version = '1.7.2'), file = paste0(plotdir172, 'summary_stats.csv'), row.names = F)
allstats172 <- as_tibble(read.csv(paste0(plotdir172, 'summary_stats.csv'), stringsAsFactors = F, header = T))
...
for (paramset in paramsets172){
  ...
  species_sample <- species %>%
    ...
    dplyr::select(A1, Aprime1, Aprime2, Anonsense1, B1, paramset, version, time, mutated_alleles)
...
write.csv(all_species_q300, file = paste0('/Volumes/IAMYG2/grn_nitc_data/v1.7.2/all_species_q300.csv'))
all_species_q300 <- as_tibble(read.csv(paste0('/Volumes/IAMYG2/grn_nitc_data/v1.7.2/all_species_q300.csv'), stringsAsFactors = F, header = T))

# summary stats draft1
allstats_full172 <- allstats172 %>% mutate(version = '1.7.2')
allparams_full172 <- allparams172 %>% mutate(paramset = 1:10000, version = '1.7.2')
lhs_sets_full172 <- lhs_sets172 %>% mutate(version = '1.7.2')

pseud = 0.01

allstats_full172 %<>% mutate(skewness = ifelse(is.na(skewness), 0, skewness))

write.csv(allstats_full172, file = paste0(plotdir172, 'summary_stats.csv'), row.names = F)

compared_stats172 <- allstats_full172 %>% 
...
unistats172<-unique(compared_stats172$stat)

for (st in unistats172) {
  
  pvs1 <- ggplot(allstats_full172 %>% inner_join(lhs_sets_full172, by = c('version', 'paramset')) %>% filter(product == 'B1', Hill_coefficient_n<5)) + 
  ...
  pvs2 <- ggplot(allstats_full172 %>% inner_join(lhs_sets_full172, by = c('version', 'paramset')) %>% filter(product == 'B1', Hill_coefficient_n<5)) + 
  ...
  pvs3 <- ggplot(allstats_full172 %>% 
                   inner_join(lhs_sets_full172, by = c('version', 'paramset')) %>% 
  ...
  ggsave(pvs1, file = paste0(plotdir172, 'PerGenotype_', st, '_vs_mean.pdf'), width = 16, height = 8) 
  ggsave(pvs2, file = paste0(plotdir172, 'PerGenotype_', st, '_vs_logmean.pdf'), width = 16, height = 8) 
  ggsave(pvs3, file = paste0(plotdir172, 'WTMUT_', st, '_vs_logmean.pdf'), width = 4, height = 4) 
  
  ggsave(pvs1, file = paste0(plotdir172, 'PerGenotype_', st, '_vs_mean.svg'), width = 16, height = 8) 
  ggsave(pvs2, file = paste0(plotdir172, 'PerGenotype_', st, '_vs_logmean.svg'), width = 16, height = 8) 
  ggsave(pvs3, file = paste0(plotdir172, 'WTMUT_', st, '_vs_logmean.svg'), width = 4, height = 4) 
}


# LOESS
loess_fitted_allstats_all172 <- allstats_full172
for (stat in unistats172[unistats172 != 'mean_product']) {
  
  cat(paste0('working on ', stat, '\n'))
  statdat <- list()
  statdat1 <- list()
  for (gene in c('A1', 'Anonsense1', 'Aprime1', 'Aprime2', 'B1')) {
    
    # for (ma in 0:2) {
    
    tempdat <- allstats_full172 %>% 
  ...
  loess_fitted_allstats_all172 %<>% left_join(as_tibble(statdat) %>% dplyr::select(-mean_product), by = c('version', 'paramset', 'mutated_alleles', 'product'))
  # 
  # cat('sliding window normalizing...\n') # do this per-gene over all genotypes, rather than all genes over all genotypes...
  # statdat1 <- sliding_window_normalize(as_tibble(statdat) %>% filter(mean_product>10), 'mean_product', paste0(stat,'_residual'), 50)
  # 
  
  loess_fitted_allstats_all172 %<>% left_join(statdat1 %>% dplyr::select(-c('mean_product', paste0(stat,'_residual'), paste0(stat,'_fitted'))), by = c('version', 'paramset', 'mutated_alleles', 'product'))
  # 
  # td1<-allstats_full1 %>% 
  #   dplyr::select(mutated_alleles, product, version, paramset, mean_product, stat)
  # 
...
write.csv(loess_fitted_allstats_all172, file = paste0(plotdir172, 'loess_fitted_allstats_all_snwRadius100.csv'), quote = F, row.names = F)

# load as needed
# loess_fitted_allstats_all172 <- as_tibble(read.csv(paste0(plotdir172, 'loess_fitted_allstats_all_snwRadius100.csv'), header=T, stringsAsFactors = F))
...
for (stat in unistats172[unistats172 != 'mean_product']) {
  ...
  for (gene in c('A1', 'Anonsense1', 'Aprime1', 'Aprime2', 'B1')) {
    ...
    statdatA = loess_fitted_allstats_all172 %>%
    ...
    pdf(paste0(paste0(plotdir172, 'LOESSplots_', stat, 'vsLogMean_', gene,'_v1.7.2.pdf')), width = 10, height = 7)
    ...
    svglite(paste0(paste0(plotdir172, 'LOESSplots_', stat, 'vsLogMean_', gene,'_v1.7.2.svg')), width = 10, height = 7)
    grid.arrange(lplot_all_stat_con,lplot_all_statLOESS_con,lplot_all_statLOESSSWN_con, ncol=3,
                 top = textGrob(paste0(stat, ' vs. log(mean_product), ', gene, '\nStat, LOESS residual, Squeezed LOESS residual (radius=50)'),gp=gpar(fontsize=20,font=3)))
    dev.off()
...
basic_class_assignment_all172 <- loess_fitted_allstats_all172 %>%
...
basic_class_assignment_all_forSankey172 <- basic_class_assignment_all172 %>%
...
set.seed(73245)
basic_class_assignment_all_forSankey_forsamples172 <- basic_class_assignment_all172 %>%
  dplyr::select(version, paramset, product, mutated_alleles, class_assignment)  

classes172 = unique(basic_class_assignment_all172$class_assignment)

if(!dir.exists(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver)))) {
  dir.create(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver)))
}

classes_sankey <- ggplot(basic_class_assignment_all_forSankey172, aes(x = mutated_alleles, y=Freq,
...
ggsave(classes_sankey, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/classes_sankey.pdf'))
ggsave(classes_sankey, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/classes_sankey.svg'))

classes_sankey172_B1forfig <- ggplot(basic_class_assignment_all_forSankey172 %>%
                                       filter(product == 'B1',
  ...
  xlab('Mutated alleles') +
  ylab('Number of parameter sets')
ggsave(classes_sankey172_B1forfig, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/classes_sankey_B1formainfig.pdf'))
ggsave(classes_sankey172_B1forfig, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/classes_sankey_B1formainfig.svg'))


basic_class_assignment_all_forpie172 <- basic_class_assignment_all172 %>%
  group_by(mutated_alleles, product, class_assignment) %>%
  summarise(nSets = length(product))

classes_pies <- ggplot(basic_class_assignment_all_forpie172, aes(x="", y=nSets, fill=class_assignment)) +
  geom_bar(stat='identity', width=1, color='white') +
  coord_polar('y', start=0) +
  facet_grid(product~mutated_alleles) +
  theme_void() +
  ggtitle('classes of all distributions in v1.7.2\neach gene in each genotype')
ggsave(classes_pies, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/classes_pies.pdf'))
ggsave(classes_pies, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/classes_pies.svg'))
...
basic_class_assignment_all172_wide <- basic_class_assignment_all172 %>% 
...
classes_for_trees172 <- inner_join(basic_class_assignment_all172_wide, lhs_sets172, by = 'paramset')

temp_class_for_tree172 <- classes_for_trees172 %>%
  dplyr::select(colnames(lhs_sets172), `1_B1`) %>% ungroup() %>%
  dplyr::select(-paramset) %>%
  mutate(is_bimodal = ifelse(`1_B1` == 'bimodal', T, F)) %>%
  dplyr::select(-`1_B1`)

temp_class_for_tree172$is_bimodal <- as.factor(temp_class_for_tree172$is_bimodal)

temp.tree <- ctree(is_bimodal ~ basal_nitc_on_ratio + onbasalA1_off_ratio + A1_Aprime1_addon_ratio + 
                     A1_Aprime_prodon_ratio + r_prod_on + r_addon_byA1_B1 + r_onbasal_A1 + Aprim1_Aprim2_prodon_ratio, 
                   data = temp_class_for_tree172,
                   control = ctree_control(alpha = 0.01))

pdf(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/isBimodalHet_tree.pdf'), width = 40, height = 10)
plot(temp.tree)
dev.off()

svglite(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/isBimodalHet_tree.svg'), width = 40, height = 10)
plot(temp.tree)
dev.off()
...
temp_class_for_tree172$is_unimodal <- as.factor(temp_class_for_tree172$is_unimodal)
...
pdf(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/isUnimodalSymmetricHet_tree.pdf'), width = 40, height = 10)
plot(temp.tree)
dev.off()

svglite(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/isUnimodalSymmetricHet_tree.svg'), width = 40, height = 10)
plot(temp.tree)
dev.off()

# decision trees log10 parameter values

classes_for_trees172_log <- inner_join(basic_class_assignment_all172_wide, 
                                       lhs_sets172 %>%
                                         mutate_at(vars(-('paramset')), log10), by = 'paramset')

temp_class_for_tree172_log <- classes_for_trees172_log %>%
  dplyr::select(colnames(lhs_sets172), `1_B1`) %>% ungroup() %>%
  dplyr::select(-paramset) %>%
  mutate(is_bimodal = ifelse(`1_B1` == 'bimodal', T, F)) %>%
  dplyr::select(-`1_B1`)

temp_class_for_tree172_log$is_bimodal <- as.factor(temp_class_for_tree172_log$is_bimodal)

temp.tree <- ctree(is_bimodal ~ basal_nitc_on_ratio + onbasalA1_off_ratio + A1_Aprime1_addon_ratio + 
                     A1_Aprime_prodon_ratio + r_prod_on + r_addon_byA1_B1 + r_onbasal_A1 + Aprim1_Aprim2_prodon_ratio, 
                   data = temp_class_for_tree172_log,
                   control = ctree_control(alpha = 0.01))

pdf(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/isBimodalHet_tree_log.pdf'), width = 40, height = 10)
plot(temp.tree)
dev.off()

svglite(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/isBimodalHet_tree_log.svg'), width = 40, height = 10)
plot(temp.tree)
dev.off()
...
temp_class_for_tree172_log %>% filter(eval(parse(text=is_bimodal_tree_splits[71])))
...
temp_class_for_tree172_log$is_unimodal <- as.factor(temp_class_for_tree172_log$is_unimodal)
...
pdf(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/isUnimodalSymmetricHet_tree_log.pdf'), width = 40, height = 10)
plot(temp.tree)
dev.off()

svglite(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/isUnimodalSymmetricHet_tree_log.svg'), width = 40, height = 10)
plot(temp.tree)
dev.off()
...
temp_class_for_tree172 <- classes_for_trees172 %>%
  ...
  dplyr::select(colnames(lhs_sets172), is_robust) %>% ungroup() %>%
  dplyr::select(-paramset) 

temp_class_for_tree172$is_robust <- as.factor(temp_class_for_tree172$is_robust)
...
pdf(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/isRobust_unimodalsymmetric_tree.pdf'), width = 20, height = 10)
plot(temp.tree)
dev.off()

svglite(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/isRobust_unimodalsymmetric_tree.svg'), width = 20, height = 10)
plot(temp.tree)
dev.off()
...
unimodal_symmetric_robust_tree172<-classes_for_trees172 %>%
  filter(`0_B1` == 'unimodal symmetric') %>%
  mutate(is_robust = ifelse(`0_B1` == `1_B1`, T, F)) %>%
  dplyr::select(colnames(lhs_sets172), is_robust) %>% ungroup() %>%
  ...
  inner_join(classes_for_trees172 %>% dplyr::select(version, paramset, `1_B1`))

unimodal_symmetric_robust_tree172_forpies <- unimodal_symmetric_robust_tree172 %>%
...
unimodal_symmetric_robust_tree172_pies <- ggplot(unimodal_symmetric_robust_tree172_forpies, aes(x="", y=fracSets, fill=`1_B1`)) +
  geom_bar(stat='identity', width=1, color='white') +
  coord_polar('y', start=0) +
  scale_fill_brewer(palette = 'Set2') +
  facet_grid(~nodeID) +
  theme_void() +
  ggtitle('classes of (B1 in heterozygous genotype) distributions in v1.7.2\neach gene in each genotype\nUnimodal symmetric in wild-type only')
ggsave(unimodal_symmetric_robust_tree172_pies, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/unimodal_symmetric_robust_tree172_pies.pdf'), width = 18, height = 4)
ggsave(unimodal_symmetric_robust_tree172_pies, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/unimodal_symmetric_robust_tree172_pies.svg'), width = 18, height = 4)

unimodal_symmetric_robust_tree172 %>%
  inner_join(compared_stats172 %>% filter(product == 'B1', stat == 'mean_product', compare == 'lfc10'), by = 'paramset') %>%
...
pdf(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/correlations_perGene_perGenotype.pdf'), width = 24, height = 32)
par(mfcol = c(5,3))
...
svglite(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/correlations_perGene_perGenotype.svg'), width = 24, height = 32)
par(mfcol = c(5,3))
for (aci in 1:length(genos)) {
  for (proi in 1:length(prods)) {
...
for (aci in 1:length(genos)) {
  ...
  for (proi in 1:length(prods)) {
    ...
    tempforcorr <- allstats_full172 %>% 
      ...
      pivot_longer(names_to = 'parameter', values_to = 'param_value', cols = basal_nitc_on_ratio:Aprim1_Aprim2_prodon_ratio)
    ...
    tempforcorr1 <- allstats_full172 %>% 
      ...
      pivot_longer(names_to = 'statistic', values_to = 'stat_value', cols = log10_mean_product1:entropy) %>%
      pivot_longer(names_to = 'parameter', values_to = 'param_value', cols = basal_nitc_on_ratio:Aprim1_Aprim2_prodon_ratio)
    ggsave(sc1, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
    ggsave(sc1, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 18)
    ...
    if(nrow(tempforcorr1)>0){
      sc1d <- ggplot(tempforcorr, aes(log10(param_value), stat_value)) +
        ...
        ylab('statistic value') +
        ggtitle(paste0('Statistic vs parameter value\nGene: ', pro, ', Genotype: ', as.character(ac), ' mutated alleles'))
      ggsave(sc1d, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
      ggsave(sc1d, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 18)
      ...
      ggsave(sc2, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
      ggsave(sc2d, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
      ggsave(sc2, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 18)
      ggsave(sc2d, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 18)
...
tempforcorr <- allstats_full172 %>% 
  ...
  filter(statistic == 'bimodality_coef',
         parameter %in% c('A1_Aprime1_addon_ratio', 'A1_Aprime_prodon_ratio', 'basal_nitc_on_ratio', 'Aprim1_Aprim2_prodon_ratio'))
...
tempforcorr1 <- allstats_full172 %>% 
  ...
  filter(statistic == 'bimodality_coef',
         parameter %in% c('A1_Aprime1_addon_ratio', 'A1_Aprime_prodon_ratio', 'basal_nitc_on_ratio', 'Aprim1_Aprim2_prodon_ratio'))
ggsave(sc1, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
ggsave(sc1, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
...
if(nrow(tempforcorr1)>0){
  sc1d <- ggplot(tempforcorr, aes(log10(param_value), stat_value)) +
    ...
    ylab('statistic value') +
    ggtitle(paste0('Statistic vs parameter value\nGene: ', pro, ', Genotype: ', as.character(ac), ' mutated alleles'))
  ggsave(sc1d, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
  ggsave(sc1d, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
  ...
  ggsave(sc2, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
  ggsave(sc2d, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
  ggsave(sc2, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
  ggsave(sc2d, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
...
c_primrat <- cor.test(tempforcortest1$bimodality_coef, log10(tempforcortest1$Aprim1_Aprim2_prodon_ratio))
c_absprimrat <- cor.test(tempforcortest1$bimodality_coef, abs(log10(tempforcortest1$Aprim1_Aprim2_prodon_ratio)))
...
cortab <- tibble(
  statistic = c('A1_Aprime1_addon_ratio', 'abs(A1_Aprime1_addon_ratio)', 'A1_Aprime_prodon_ratio', 'abs(A1_Aprime_prodon_ratio)', 
                'basal_nitc_on_ratio', 'abs(basal_nitc_on_ratio)', 'Aprim1_Aprim2_prodon_ratio', 'abs(Aprim1_Aprim2_prodon_ratio)'),#, 'r_onbasal_Aprime_ratio', 'abs(r_onbasal_Aprime_ratio)'),
  cor = c(c_addon$estimate, c_absaddon$estimate, c_prodon$estimate, c_absprodon$estimate, c_nitc$estimate, c_absnitc$estimate, c_primrat$estimate, c_absprimrat$estimate),#, c_primon$estimate, c_absprimon$estimate),
  p.val = c(c_addon$p.value, c_absaddon$p.value, c_prodon$p.value, c_absprodon$p.value, c_nitc$p.value, c_absnitc$p.value, c_primrat$p.value, c_absprimrat$p.value)#, c_primon$p.value, c_absprimon$p.value)
)
write.csv(cortab, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_pearsonCor.csv'), quote = F, row.names = F)
...
for (aci in 1:length(genos)) {
  ...
  for (proi in 1:length(prods)) {
    ...
    cat(paste0('Working on scatters for ', pro, ' in genotype ', ac, '\n'))
    tempforcorr_swn <- loess_fitted_allstats_all172 %>% 
      filter(mutated_alleles == ac,
             product == pro) %>%
      ungroup() %>%
      mutate(log10_mean_product1 = log10(mean_product+1)) %>%
    ...
    tempforcorr1_swn <- loess_fitted_allstats_all172 %>% 
      filter(mutated_alleles == ac,
             product == pro,
             mean_product > 10) %>%
      ungroup() %>%
    ...
    if(nrow(tempforcorr1_swn)>0){
      ...
      sc1d_swn <- ggplot(tempforcorr_swn, aes(log10(param_value), stat_value)) +
        ...
        ylab('statistic value') +
        ggtitle(paste0('Statistic vs normalized LOESS residual of parameter value\nGene: ', pro, ', Genotype: ', as.character(ac), ' mutated alleles'))
      ggsave(sc1_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 15)
      ggsave(sc1d_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 15)
      ggsave(sc1_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 15)
      ggsave(sc1d_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 15)
      ...
      ggsave(sc2_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 15)
      ggsave(sc2d_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 15)
      ggsave(sc2_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 15)
      ggsave(sc2d_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 15)
...
tempforcorr1_swn <- loess_fitted_allstats_all172 %>% 
  ...
  filter(statistic == 'bimodality_coef',
         parameter %in% c('A1_Aprime1_addon_ratio', 'A1_Aprime_prodon_ratio', 'basal_nitc_on_ratio', 'Aprim1_Aprim2_prodon_ratio'))
ggsave(sc1_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
ggsave(sc1_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
...
if(nrow(tempforcorr1)>0){
  sc1d_swn <- ggplot(tempforcorr_swn, aes(log10(param_value), stat_value)) +
    ...
    ylab('statistic value') +
    ggtitle(paste0('Statistic vs LOESS residual of parameter value\nGene: ', pro, ', Genotype: ', as.character(ac), ' mutated alleles'))
  ggsave(sc1d, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
  ggsave(sc1d, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
  ...
  ggsave(sc2_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
  ggsave(sc2d_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
  ggsave(sc2_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
  ggsave(sc2d_swn, file = paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
...
comparisons = unique(compared_stats172$compare)
for(comp in comparisons) {
  pdf(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/correlations_',comp,'_perGene_perGenotype.pdf'), width = 12, height = 12)
  par(mfrow = c(2,2))
  for (proi in 1:length(prods)) {
    
    pro = prods[proi]
    
    tempforcorr <- compared_stats172 %>% 
      filter(compare == comp) %>% 
...
for(comp in comparisons) {
  svglite(paste0(plotdir172, 'stats_class_assignment_check_v', as.character(anver),'/correlations_',comp,'_perGene_perGenotype.svg'), width = 12, height = 12)
  par(mfrow = c(2,2))
  for (proi in 1:length(prods)) {

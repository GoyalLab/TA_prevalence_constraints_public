...
# edit as needed
datadir171 <- '/Volumes/IAMYG2/grn_nitc_data/v1.7.1/samples/'
plotdir171 <- '/Volumes/IAMYG2/grn_nitc_data/v1.7.1/exploratory_analysis/'
setwd(datadir171)

if(!dir.exists(plotdir171)){
  dir.create(plotdir171)
}
paramsets171 <- 1:10000
lhs_sets171 <- as_tibble(read.csv('latinhyp_sampledSets.csv')) 
lhs_sets171 %<>%
  mutate(paramset = 1:nrow(lhs_sets171))

# calculate stats
allstats171 <- list()
allparams171 <- list()
for (paramset in paramsets171){
  ...
  if(paramset %% 100 == 0) {
    ...
    #   theme_classic()
    # ggsave(dist_plot, file = paste0(plotdir171, 'distributions_q300_v1.7.1_paramset_', as.character(paramset), '.pdf'))
  }
  ...
  if(is.null(dim(allstats171))) {
    allstats171 <- spstats
  } else {
    allstats171 %<>% bind_rows(spstats)
  }
  
  if(is.null(dim(allparams171))) {
    allparams171 <- params
  } else {
    allparams171 %<>% bind_rows(params)
  }
  
}
write.csv(allstats171 %>% mutate(version = '1.7.1'), file = paste0(plotdir171, 'summary_stats.csv'), row.names = F)
allstats171 <- as_tibble(read.csv(paste0(plotdir171, 'summary_stats.csv'), stringsAsFactors = F, header = T))
...
allparams_full171 <- allparams171 %>% mutate(paramset = 1:10000, version = '1.7.1')
lhs_sets_full171 <- lhs_sets171 %>% mutate(version = '1.7.1')

pseud = 0.01

allstats_full171 %<>% mutate(skewness = ifelse(is.na(skewness), 0, skewness))

write.csv(allstats_full171, file = paste0(plotdir171, 'summary_stats.csv'), row.names = F)

compared_stats171 <- allstats_full171 %>% 
...
unistats171<-unique(compared_stats171$stat)

for (st in unistats171) {
  
  pvs1 <- ggplot(allstats_full171 %>% inner_join(lhs_sets_full171, by = c('version', 'paramset')) %>% filter(product == 'B1', Hill_coefficient_n<5)) + 
  ...
  pvs2 <- ggplot(allstats_full171 %>% inner_join(lhs_sets_full171, by = c('version', 'paramset')) %>% filter(product == 'B1', Hill_coefficient_n<5)) + 
  ...
  pvs3 <- ggplot(allstats_full171 %>% 
                   inner_join(lhs_sets_full171, by = c('version', 'paramset')) %>% 
  ...
  ggsave(pvs1, file = paste0(plotdir171, 'PerGenotype_', st, '_vs_mean.pdf'), width = 16, height = 8) 
  ggsave(pvs2, file = paste0(plotdir171, 'PerGenotype_', st, '_vs_logmean.pdf'), width = 16, height = 8) 
  ggsave(pvs3, file = paste0(plotdir171, 'WTMUT_', st, '_vs_logmean.pdf'), width = 4, height = 4) 
  
  ggsave(pvs1, file = paste0(plotdir171, 'PerGenotype_', st, '_vs_mean.svg'), width = 16, height = 8) 
  ggsave(pvs2, file = paste0(plotdir171, 'PerGenotype_', st, '_vs_logmean.svg'), width = 16, height = 8) 
  ggsave(pvs3, file = paste0(plotdir171, 'WTMUT_', st, '_vs_logmean.svg'), width = 4, height = 4) 
}


# LOESS
loess_fitted_allstats_all171 <- allstats_full171
for (stat in unistats171[unistats171 != 'mean_product']) {
  ...
  loess_fitted_allstats_all171 %<>% left_join(as_tibble(statdat) %>% dplyr::select(-mean_product), by = c('version', 'paramset', 'mutated_alleles', 'product'))
  # 
  # cat('sliding window normalizing...\n') # do this per-gene over all genotypes, rather than all genes over all genotypes...
  # statdat1 <- sliding_window_normalize(as_tibble(statdat) %>% filter(mean_product>10), 'mean_product', paste0(stat,'_residual'), 50)
  # 
  
  loess_fitted_allstats_all171 %<>% left_join(statdat1 %>% dplyr::select(-c('mean_product', paste0(stat,'_residual'), paste0(stat,'_fitted'))), by = c('version', 'paramset', 'mutated_alleles', 'product'))
  # 
  # td1<-allstats_full1 %>% 
  #   dplyr::select(mutated_alleles, product, version, paramset, mean_product, stat)
  # 
...
write.csv(loess_fitted_allstats_all171, file = paste0(plotdir171, 'loess_fitted_allstats_all_snwRadius100.csv'), quote = F, row.names = F)

# load as needed
# loess_fitted_allstats_all171 <- as_tibble(read.csv(paste0(plotdir171, 'loess_fitted_allstats_all_snwRadius100.csv'), header=T, stringsAsFactors = F))
...
for (stat in unistats171[unistats171 != 'mean_product']) {
  ...
  for (gene in c('A1', 'Anonsense1', 'Aprime1', 'Aprime2', 'B1')) {
    ...
    statdatA = loess_fitted_allstats_all171 %>%
    ...
    pdf(paste0(paste0(plotdir171, 'LOESSplots_', stat, 'vsLogMean_', gene,'_v1.7.1.pdf')), width = 10, height = 7)
    ...
    svglite(paste0(paste0(plotdir171, 'LOESSplots_', stat, 'vsLogMean_', gene,'_v1.7.1.svg')), width = 10, height = 7)
    grid.arrange(lplot_all_stat_con,lplot_all_statLOESS_con,lplot_all_statLOESSSWN_con, ncol=3,
                 top = textGrob(paste0(stat, ' vs. log(mean_product), ', gene, '\nStat, LOESS residual, Squeezed LOESS residual (radius=50)'),gp=gpar(fontsize=20,font=3)))
    dev.off()
...
basic_class_assignment_all171 <- loess_fitted_allstats_all171 %>%
...
basic_class_assignment_all_forSankey171 <- basic_class_assignment_all171 %>%
...
set.seed(73245)
basic_class_assignment_all_forSankey_forsamples171 <- basic_class_assignment_all171 %>%
  dplyr::select(version, paramset, product, mutated_alleles, class_assignment)  

classes171 = unique(basic_class_assignment_all171$class_assignment)

if(!dir.exists(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver)))) {
  dir.create(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver)))
}

classes_sankey <- ggplot(basic_class_assignment_all_forSankey171, aes(x = mutated_alleles, y=Freq,
...
ggsave(classes_sankey, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/classes_sankey.pdf'))
ggsave(classes_sankey, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/classes_sankey.svg'))

classes_sankey171_B1forfig <- ggplot(basic_class_assignment_all_forSankey171 %>%
                                      filter(product == 'B1',
  ...
  xlab('Mutated alleles') +
  ylab('Number of parameter sets')
ggsave(classes_sankey171_B1forfig, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/classes_sankey_B1formainfig.pdf'))
ggsave(classes_sankey171_B1forfig, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/classes_sankey_B1formainfig.svg'))


basic_class_assignment_all_forpie171 <- basic_class_assignment_all171 %>%
  group_by(mutated_alleles, product, class_assignment) %>%
  summarise(nSets = length(product))

classes_pies <- ggplot(basic_class_assignment_all_forpie171, aes(x="", y=nSets, fill=class_assignment)) +
  geom_bar(stat='identity', width=1, color='white') +
  coord_polar('y', start=0) +
  facet_grid(product~mutated_alleles) +
  theme_void() +
  ggtitle('classes of all distributions in v1.7.1\neach gene in each genotype')
ggsave(classes_pies, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/classes_pies.pdf'))
ggsave(classes_pies, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/classes_pies.svg'))
...
basic_class_assignment_all171_wide <- basic_class_assignment_all171 %>% 
...
classes_for_trees171 <- inner_join(basic_class_assignment_all171_wide, lhs_sets171, by = 'paramset')

temp_class_for_tree171 <- classes_for_trees171 %>%
  dplyr::select(colnames(lhs_sets171), `1_B1`) %>% ungroup() %>%
  dplyr::select(-paramset) %>%
  mutate(is_bimodal = ifelse(`1_B1` == 'bimodal', T, F)) %>%
  dplyr::select(-`1_B1`)

temp_class_for_tree171$is_bimodal <- as.factor(temp_class_for_tree171$is_bimodal)
...
temp.tree <- ctree(is_bimodal ~ basal_nitc_on_ratio + onbasalA1_off_ratio + A1_Aprime1_addon_ratio + 
                   ...
                   data = temp_class_for_tree171,
                   control = ctree_control(alpha = 0.01))

pdf(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/isBimodalHet_tree.pdf'), width = 40, height = 10)
plot(temp.tree)
dev.off()

svglite(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/isBimodalHet_tree.svg'), width = 40, height = 10)
plot(temp.tree)
dev.off()
...
temp_class_for_tree171$is_unimodal <- as.factor(temp_class_for_tree171$is_unimodal)
...
pdf(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/isUnimodalSymmetricHet_tree.pdf'), width = 40, height = 10)
plot(temp.tree)
dev.off()

svglite(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/isUnimodalSymmetricHet_tree.svg'), width = 40, height = 10)
plot(temp.tree)
dev.off()

# decision trees log10 parameter values

classes_for_trees171_log <- inner_join(basic_class_assignment_all171_wide, 
                                      lhs_sets171 %>%
                                        mutate_at(vars(-('paramset')), log10), by = 'paramset')

temp_class_for_tree171_log <- classes_for_trees171_log %>%
  dplyr::select(colnames(lhs_sets171), `1_B1`) %>% ungroup() %>%
  dplyr::select(-paramset) %>%
  mutate(is_bimodal = ifelse(`1_B1` == 'bimodal', T, F)) %>%
  dplyr::select(-`1_B1`)

temp_class_for_tree171_log$is_bimodal <- as.factor(temp_class_for_tree171_log$is_bimodal)

temp.tree <- ctree(is_bimodal ~ basal_nitc_on_ratio + onbasalA1_off_ratio + A1_Aprime1_addon_ratio + 
                     A1_Aprime_prodon_ratio + r_prod_on + r_addon_byA1_B1 + r_onbasal_A1, 
                   data = temp_class_for_tree171_log,
                   control = ctree_control(alpha = 0.01))

pdf(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/isBimodalHet_tree_log.pdf'), width = 40, height = 10)
plot(temp.tree)
dev.off()

svglite(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/isBimodalHet_tree_log.svg'), width = 40, height = 10)
plot(temp.tree)
dev.off()
...
temp_class_for_tree171_log %>% filter(eval(parse(text=is_bimodal_tree_splits[71])))
...
temp_class_for_tree171_log$is_unimodal <- as.factor(temp_class_for_tree171_log$is_unimodal)
...
pdf(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/isUnimodalSymmetricHet_tree_log.pdf'), width = 40, height = 10)
plot(temp.tree)
dev.off()

svglite(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/isUnimodalSymmetricHet_tree_log.svg'), width = 40, height = 10)
plot(temp.tree)
dev.off()
...
temp_class_for_tree171 <- classes_for_trees171 %>%
  ...
  dplyr::select(colnames(lhs_sets171), is_robust) %>% ungroup() %>%
  dplyr::select(-paramset) 

temp_class_for_tree171$is_robust <- as.factor(temp_class_for_tree171$is_robust)
...
pdf(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/isRobust_unimodalsymmetric_tree.pdf'), width = 20, height = 10)
plot(temp.tree)
dev.off()

svglite(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/isRobust_unimodalsymmetric_tree.svg'), width = 20, height = 10)
plot(temp.tree)
dev.off()
...
unimodal_symmetric_robust_tree171<-classes_for_trees171 %>%
  filter(`0_B1` == 'unimodal symmetric') %>%
  mutate(is_robust = ifelse(`0_B1` == `1_B1`, T, F)) %>%
  dplyr::select(colnames(lhs_sets171), is_robust) %>% ungroup() %>%
  ...
  inner_join(classes_for_trees171 %>% dplyr::select(version, paramset, `1_B1`))

unimodal_symmetric_robust_tree171_forpies <- unimodal_symmetric_robust_tree171 %>%
...
unimodal_symmetric_robust_tree171_pies <- ggplot(unimodal_symmetric_robust_tree171_forpies, aes(x="", y=fracSets, fill=`1_B1`)) +
  geom_bar(stat='identity', width=1, color='white') +
  coord_polar('y', start=0) +
  scale_fill_brewer(palette = 'Set2') +
  facet_grid(~nodeID) +
  theme_void() +
  ggtitle('classes of (B1 in heterozygous genotype) distributions in v1.7.1\neach gene in each genotype\nUnimodal symmetric in wild-type only')
ggsave(unimodal_symmetric_robust_tree171_pies, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/unimodal_symmetric_robust_tree171_pies.pdf'), width = 18, height = 4)
ggsave(unimodal_symmetric_robust_tree171_pies, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/unimodal_symmetric_robust_tree171_pies.svg'), width = 18, height = 4)

unimodal_symmetric_robust_tree171 %>%
  inner_join(compared_stats171 %>% filter(product == 'B1', stat == 'mean_product', compare == 'lfc10'), by = 'paramset') %>%
...
svglite(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/correlations_perGene_perGenotype.svg'), width = 24, height = 32)
par(mfcol = c(5,3))
for (aci in 1:length(genos)) {
  for (proi in 1:length(prods)) {
...
for (aci in 1:length(genos)) {
  ...
  for (proi in 1:length(prods)) {
    ...
    tempforcorr1 <- allstats_full171 %>% 
      ...
      pivot_longer(names_to = 'statistic', values_to = 'stat_value', cols = log10_mean_product1:entropy) %>%
      pivot_longer(names_to = 'parameter', values_to = 'param_value', cols = basal_nitc_on_ratio:r_onbasal_A1)
    ggsave(sc1, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
    ggsave(sc1, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 18)
    ...
    if(nrow(tempforcorr1)>0){
      sc1d <- ggplot(tempforcorr, aes(log10(param_value), stat_value)) +
        ...
        ylab('statistic value') +
        ggtitle(paste0('Statistic vs parameter value\nGene: ', pro, ', Genotype: ', as.character(ac), ' mutated alleles'))
      ggsave(sc1d, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
      ggsave(sc1d, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 18)
      ...
      ggsave(sc2, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
      ggsave(sc2d, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 18)
      ggsave(sc2, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 18)
      ggsave(sc2d, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 18)
...
tempforcorr1 <- allstats_full171 %>% 
  ...
  filter(statistic == 'bimodality_coef',
         parameter %in% c('A1_Aprime1_addon_ratio', 'A1_Aprime_prodon_ratio', 'basal_nitc_on_ratio'))
ggsave(sc1, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
ggsave(sc1, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
...
if(nrow(tempforcorr1)>0){
  sc1d <- ggplot(tempforcorr, aes(log10(param_value), stat_value)) +
    ...
    ylab('statistic value') +
    ggtitle(paste0('Statistic vs parameter value\nGene: ', pro, ', Genotype: ', as.character(ac), ' mutated alleles'))
  ggsave(sc1d, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
  ggsave(sc1d, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
  ...
  ggsave(sc2, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
  ggsave(sc2d, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
  ggsave(sc2, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
  ggsave(sc2d, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
...
write.csv(cortab, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_pearsonCor.csv'), quote = F, row.names = F)
...
for (aci in 1:length(genos)) {
  ...
  for (proi in 1:length(prods)) {
    ...
    cat(paste0('Working on scatters for ', pro, ' in genotype ', ac, '\n'))
    tempforcorr_swn <- loess_fitted_allstats_all171 %>% 
      filter(mutated_alleles == ac,
             product == pro) %>%
      ungroup() %>%
      mutate(log10_mean_product1 = log10(mean_product+1)) %>%
    ...
    tempforcorr1_swn <- loess_fitted_allstats_all171 %>% 
      filter(mutated_alleles == ac,
             product == pro,
             mean_product > 10) %>%
      ungroup() %>%
    ...
    if(nrow(tempforcorr1_swn)>0){
      ...
      sc1d_swn <- ggplot(tempforcorr_swn, aes(log10(param_value), stat_value)) +
        ...
        ylab('statistic value') +
        ggtitle(paste0('Statistic vs normalized LOESS residual of parameter value\nGene: ', pro, ', Genotype: ', as.character(ac), ' mutated alleles'))
      ggsave(sc1_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 15)
      ggsave(sc1d_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 15)
      ggsave(sc1_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 15)
      ggsave(sc1d_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 15)
      ...
      ggsave(sc2_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 15)
      ggsave(sc2d_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.pdf'), width = 24, height = 15)
      ggsave(sc2_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 15)
      ggsave(sc2d_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'.svg'), width = 24, height = 15)
...
tempforcorr1_swn <- loess_fitted_allstats_all171 %>% 
  ...
  filter(statistic == 'bimodality_coef',
         parameter %in% c('A1_Aprime1_addon_ratio', 'A1_Aprime_prodon_ratio', 'basal_nitc_on_ratio'))
ggsave(sc1_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
ggsave(sc1_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
...
if(nrow(tempforcorr1)>0){
  sc1d_swn <- ggplot(tempforcorr_swn, aes(log10(param_value), stat_value)) +
    ...
    ylab('statistic value') +
    ggtitle(paste0('Statistic vs LOESS residual of parameter value\nGene: ', pro, ', Genotype: ', as.character(ac), ' mutated alleles'))
  ggsave(sc1d, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
  ggsave(sc1d, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
  ...
  ggsave(sc2_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
  ggsave(sc2d_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.pdf'), width = 12, height = 4)
  ggsave(sc2_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatter_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
  ggsave(sc2d_swn, file = paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/stats_swn_scatterDensity_minMean10_gene', pro, '_mutatedalleles', as.character(ac),'_B1hetfocus.svg'), width = 12, height = 4)
...
comparisons = unique(compared_stats171$compare)
for(comp in comparisons) {
  pdf(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/correlations_',comp,'_perGene_perGenotype.pdf'), width = 12, height = 12)
  par(mfrow = c(2,2))
  for (proi in 1:length(prods)) {
    
    pro = prods[proi]
    
    tempforcorr <- compared_stats171 %>% 
      filter(compare == comp) %>% 
...
for(comp in comparisons) {
  svglite(paste0(plotdir171, 'stats_class_assignment_check_v', as.character(anver),'/correlations_',comp,'_perGene_perGenotype.svg'), width = 12, height = 12)
  par(mfrow = c(2,2))
  for (proi in 1:length(prods)) {
